...
bool Superaccumulator::CompareSuperaccumulatorWithMPFR(char *res_mpfr) {
  mpfr_t result;
  mpfr_exp_t exp_ptr;
  ...
  printf("\tSum Superaccumulator (52)   : %s \t e%d\n", res_str, (int) exp_ptr);
...
bool Superaccumulator::CompareSuperaccumulatorWithMPFROld(char *res_mpfr) {
  mpfr_t result;
  mpfr_exp_t exp_ptr;
  ...
  for (int j = 0; j < (e_words + f_words); j++) {
    l2 = accumulator[j];
    l1 = (accumulator[j] - l2) >> 32;
    printf("[%d] acc[i] = %lX \t l1 = %X \t l2 = %X\n", j, accumulator[j], l1,
           l2);

    if (l1 == 0)
      d1 = 0.;
    else
      d1 = ldexp(l1, (j - f_words) * digits + 32);
    if (l2 == 0)
      d2 = 0.;
    else
      d2 = ldexp(l2, (j - f_words) * digits);

    mpfr_add_d(result, result, d1, MPFR_RNDN);
    mpfr_add_d(result, result, d2, MPFR_RNDN);
  }
