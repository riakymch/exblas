...
int main(int argc, char *argv[]) {
    ...
    bool is_pass = true;
    double exsum_acc, exsum_fpe2, exsum_fpe3, exsum_fpe4, exsum_fpe8, exsum_fpe8ee;
    exsum_acc = exsum(N, a, 1, 0);
    exsum_fpe2 = exsum(N, a, 1, 2);
    exsum_fpe3 = exsum(N, a, 1, 3);
    exsum_fpe4 = exsum(N, a, 1, 4);
    exsum_fpe8 = exsum(N, a, 1, 8);
    ...
    if ((exsum_acc > eps) || (exsum_fpe2 > eps) || (exsum_fpe4 > eps) || (exsum_fpe3 > eps) || (exsum_fpe8 > eps) || (exsum_fpe8ee > eps)) {
        is_pass = false;
        printf("FAILED: %.16g %.16g \t %.16g \t %.16g \t %.16g \t %.16g\n", exsum_acc, exsum_fpe2, exsum_fpe3, exsum_fpe4, exsum_fpe8, exsum_fpe8ee);
    }
#else
    ...
    if ((exsum_fpe2 > eps) || (exsum_fpe4 > eps) || (exsum_fpe3 > eps) || (exsum_fpe8 > eps) || (exsum_fpe8ee > eps)) {
        is_pass = false;
        printf("FAILED: %.16g \t %.16g \t %.16g \t %.16g \t %.16g\n", exsum_fpe2, exsum_fpe3, exsum_fpe4, exsum_fpe8, exsum_fpe8ee);
    }
#endif
    fprintf(stderr, "\n");
