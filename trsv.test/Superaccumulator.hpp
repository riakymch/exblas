...
struct Superaccumulator
{
    ...
    void Reset(int e_bits = 1023, int f_bits = 1023 + 52);
    void Accumulate(int64_t x, int exp);
    void Accumulate(double x);
    void Accumulate(Superaccumulator & other);   // May modify (normalize) other member
    double Round();
    bool CompareSuperaccumulatorWithMPFR(mpfr_t *res_mpfr);
    bool CompareRoundedResults(mpfr_t *res_mpfr, double res_rounded);
