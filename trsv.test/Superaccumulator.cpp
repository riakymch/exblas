...
void Superaccumulator::Reset(int e_bits, int f_bits){
    K = 8;
    digits = 64 - K;
    deltaScale = double(1ull << digits);
    f_words = (f_bits + digits - 1) / digits;
    e_words = (e_bits + digits - 1) / digits;
    for (int i = 0; i < (f_words + e_words); i++) {
        accumulator[i] = 0;
    }

    imin = f_words + e_words - 1;
    imax = 0;
    status = Exact;
...
bool Superaccumulator::CompareSuperaccumulatorWithMPFR(mpfr_t *res_mpfr) {
  mpfr_t res_superacc, res_iter;
  mpfr_exp_t exp_ptr;

  mpfr_init2(res_superacc, 2098);
  mpfr_init2(res_iter, 128);
  mpfr_set_d(res_superacc, 0.0, MPFR_RNDN);
  ...
  for (int j = 0; j < (e_words + f_words); j++) {
    ...
    //scale each bin of superacc
    mpfr_set_si_2exp(res_iter, x, (j - f_words) * digits, MPFR_RNDN);

    //add scaled value to total result
    mpfr_add(res_superacc, res_superacc, res_iter, MPFR_RNDN);
  }

  char *str_mpfr = mpfr_get_str(NULL, &exp_ptr, 10, 2098, *res_mpfr, MPFR_RNDD);
  char *str_superacc = mpfr_get_str(NULL, &exp_ptr, 10, 2098, res_superacc, MPFR_RNDD);
  printf("\tSum MPFR (2098)    : %s\n", str_mpfr);
  printf("\tSum Superaccumulator (2098) : %s\n", str_superacc);
  mpfr_free_str(str_mpfr);
  mpfr_free_str(str_superacc);

  //Bit-wise comparision of the results using the MPFR native functions
  bool res_cmp = false;
  int cmp = mpfr_cmp(*res_mpfr, res_superacc);
  if (cmp == 0){
  ...
  //Compare rounded values of MPFR and superaccumulator
  double rounded_mpfr = mpfr_get_d(*res_mpfr, MPFR_RNDD);
  double rounded_superacc = mpfr_get_d(res_superacc, MPFR_RNDD);
  printf("Rounded value of MPFR: %.17g\n", rounded_mpfr);
  printf("Rounded value of superaccumulator: %.17g\n", rounded_superacc);
  if (rounded_mpfr == rounded_superacc){
      printf("\t The result is EXACT -- matches the MPFR algorithm!\n\n");
      res_cmp = true;
  } else {
      printf("\t The result is WRONG -- does not match the MPFR algorithm!\n\n");
