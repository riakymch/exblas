...
oclblas_smake(
    ...
    int n, float *a, int nmax, float *aa, int lda,
    int *reset, float transl
    )
{
  /* Local variables */
  static int i, j;
  ...
  for (j = 1; j <= n; ++j) {
    for (i = 1; i <= m; ++i) {
      if (gen || (upper && i <= j) || (lower && i >= j)) {
        a[i + j * nmax] = oclblas_sbeg(reset) + transl;
        if (i != j) {
