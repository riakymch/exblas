...
void AccumulateWord(volatile long *sa, int lda, int i, long x) {
...
void Accumulate(volatile long *sa, int lda, double x) {
    if (x == 0)
        return;
...
__kernel void trsv_lnn(
    ...
    long d_Superaccs[BIN_COUNT];
    long *l_working = d_Superaccs;
