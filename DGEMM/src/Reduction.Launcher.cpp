...
extern "C" cl_int initReduction(
    ...
    printf("...allocating internal buffer\n");
        d_PartialAccumulators = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE, PARTIAL_ACCUMULATORS_COUNT * sizeof(cl_double), NULL, &ciErrNum);
        if (ciErrNum != CL_SUCCESS) {
...
extern "C" size_t Reduction(
...

    {
        ...
        ciErrNum  = clSetKernelArg(ckKernel, 0, sizeof(cl_mem),  (void *)&d_PartialAccumulators);
        ciErrNum |= clSetKernelArg(ckKernel, 1, sizeof(cl_mem),  (void *)&d_iData);
        ciErrNum |= clSetKernelArg(ckKernel, 2, sizeof(cl_uint), (void *)&NbElements);
        if (ciErrNum != CL_SUCCESS) {
    ...
    }
    {
        ...
        ciErrNum  = clSetKernelArg(ckComplete, 0, sizeof(cl_mem),  (void *)&d_oData);
        ciErrNum |= clSetKernelArg(ckComplete, 1, sizeof(cl_mem),  (void *)&d_PartialAccumulators);
        ciErrNum |= clSetKernelArg(ckComplete, 2, sizeof(cl_uint), (void *)&PARTIAL_ACCUMULATORS_COUNT);
        if (ciErrNum != CL_SUCCESS) {
            printf("Error in clSetKernelArg, Line %u in file %s !!!\n\n", __LINE__, __FILE__);
	    *ciErrNumRes = EXIT_FAILURE;
            return 0;
