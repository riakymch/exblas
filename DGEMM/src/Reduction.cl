...
void Reduction(
    __global data_t *d_PartialSummation,
    ...
    if (lid == 0)
        d_PartialSummation[get_group_id(0)] = l_sa[0];
...
void ReductionComplete(
    ...
    for(uint i = lid; i < NbElements; i += MERGE_WORKGROUP_SIZE)
        sum += d_PartialSummation[gid * MERGE_WORKGROUP_SIZE + i];
    l_Data[lid] = sum;
